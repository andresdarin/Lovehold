...
const TOOL_NAMES = ['get_product_guide', 'get_monthly_activity', 'get_accounts', 'get_financial_snapshot', 'get_spending_capacity', 'get_upcoming_obligations', 'simulate_purchase', 'create_expense']
...
export class AiConfigResolver {
  ...
  async resolve(agentSlug = 'finnic', environment = this.config?.get('AI_ENV') || process.env.AI_ENV || 'PROD'): Promise<EffectiveAiConfig> {
    ...
    try {
      ...
      const rows = await (this.prisma as any).aiToolConfig.findMany({ where: { agentId: agent.id } })
      const base = fallback(agent.id)
      const tools = rows.length ? rows.map((t: any) => ({ name: t.toolName, enabled: t.enabled, requireConfirmation: t.requireConfirmation, maxAttempts: t.maxAttempts })) : base.tools
      const value: EffectiveAiConfig = { agent: { id: agent.id, slug: agent.slug, name: agent.name }, prompt: prompt ? { id: prompt.id, key: prompt.prompt?.key || 'finnic-system', content: prompt.content, version: prompt.version } : base.prompt, model: model ? { model: model.model, temperature: model.temperature, maxTokens: model.maxTokens, responseMimeType: model.responseMimeType || undefined } : base.model, tools, policy: base.policy }
      return this.store(key, value)
    } catch { throw new ServiceUnavailableException('No pudimos cargar la configuración de Finnic.') }
  }
