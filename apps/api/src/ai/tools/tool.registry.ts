...
const declarations: Declaration[] = [
  { name: 'get_monthly_activity', description: 'Consulta totales de ingresos y egresos personales del mes completo, categorías por moneda y hasta 30 movimientos recientes. No incluye hogar ni convierte monedas.', parameters: { type: 'object', properties: { month: { type: 'string', description: 'Mes YYYY-MM. Omitir para mes actual en zona horaria del perfil.' } }, additionalProperties: false } },
  { name: 'get_accounts', description: 'Lista cuentas propias activas, nombres, monedas, saldos registrados y tipos. Solo lectura, sin sincronización bancaria.', parameters: { type: 'object', properties: {}, additionalProperties: false } },
  { name: 'get_product_guide', description: 'Explica funcionalidades reales, rutas y límites de Finnic.', parameters: { type: 'object', properties: {}, additionalProperties: false } },
  ...
  { name: 'create_expense', description: 'Registra un gasto personal en las finanzas del perfil', parameters: { type: 'object', required: ['amount', 'currency', 'category', 'title', 'financeAccountId'], properties: { amount: { type: 'number', exclusiveMinimum: 0 }, currency: { type: 'string', enum: ['UYU', 'USD'], default: 'UYU' }, category: { type: 'string', minLength: 1 }, title: { type: 'string', minLength: 1 }, date: { type: 'string' }, notes: { type: 'string' }, financeAccountId: { type: 'string' } } } },
]
