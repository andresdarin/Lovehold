...
describe('AI backoffice Fase 4', () => {
  it('1. hides AI navigation for non-admins', () => { const { rerender } = render(React.createElement(SidebarNav, navProps(false))); expect(screen.queryByRole('link', { name: /ai/i })).toBeNull(); rerender(React.createElement(SidebarNav, navProps(true))); expect(screen.getByRole('link', { name: /ai/i })).toBeTruthy() })
  it('2. changes effective config with environment', () => { mocks.config.mockImplementation((env: string) => ({ data: { model: { model: env === 'development' ? 'dev-model' : 'prod-model' }, prompt: { key: env, version: 1 }, tools: [] }, loading: false })); const { rerender } = render(React.createElement(AiOverview)); expect(screen.getByText('dev-model')).toBeTruthy(); fireEvent.change(screen.getByLabelText('environment'), { target: { value: 'production' } }); rerender(React.createElement(AiOverview)); expect(screen.getByText('prod-model')).toBeTruthy() })
  it('3. keeps draft separate from published/deployed', () => { mocks.prompts.mockReturnValue({ data: [{ id: 'p1', key: 'finnic-system' }], loading: false }); mocks.lifecycle.mockReturnValue({ versions: { data: [{ version: 2, status: 'draft' }, { version: 1, status: 'published' }] }, draft: { version: 2 }, published: { version: 1 }, activeByEnv: { development: { version: 1 }, test: { version: 1 }, production: { version: 1 } } }); render(React.createElement(BehaviorPage)); expect(screen.getByText('Draft · v2')).toBeTruthy(); expect(screen.getByText('Published · v1')).toBeTruthy(); expect(screen.getAllByText('Active · v1').length).toBe(3) })
  it('4. publishing does not change active deployment', async () => { mocks.lifecycle.mockReturnValue({ versions: { data: [], loading: false }, draft: { version: 2, content: 'draft' }, published: { version: 1 }, activeByEnv: { development: { version: 1 }, test: { version: 1 }, production: { version: 1 } } }); render(React.createElement(BehaviorPage)); fireEvent.click(screen.getAllByRole('button', { name: /publish version/i })[0]!); await waitFor(() => expect(mocks.mutation).toHaveBeenCalled()); expect(screen.getAllByText('Active · v1').length).toBe(3) })
  it('5. deployment mutation is the transition to active', async () => { mocks.deployments.mockReturnValue({ data: [{ id: 'd1', isActive: false, promptVersion: { version: 2 } }], loading: false, refresh: vi.fn() }); mocks.config.mockReturnValue({ data: { prompt: { version: 1 } }, loading: false, refresh: vi.fn() }); render(React.createElement(VersionsPage)); fireEvent.click(screen.getByRole('button', { name: /deploy active/i })); await waitFor(() => expect(mocks.mutation).toHaveBeenCalledWith('/ai/admin/deployments', 'POST', { environment: 'dev' })) })
  it('6. requires PROD confirmation for rollback', async () => { mocks.deployments.mockReturnValue({ data: [{ id: 'd1', isActive: true, promptVersion: { version: 1 } }], loading: false, refresh: vi.fn() }); mocks.config.mockReturnValue({ data: { prompt: { version: 1 } }, loading: false, refresh: vi.fn() }); render(React.createElement(VersionsPage)); fireEvent.change(screen.getByLabelText('environment'), { target: { value: 'production' } }); fireEvent.click(screen.getByRole('button', { name: /rollback/i })); expect(mocks.mutation).not.toHaveBeenCalled(); expect(screen.getByText('Escribe PROD para confirmar el rollback.')).toBeTruthy() })
  it('7. playground never executes a write tool', async () => { const send = vi.fn(); mocks.playground.mockReturnValue({ send, sending: false, error: null, result: { response: 'ok', toolCalls: [{ name: 'create_expense', risk: 'write' }] } }); render(React.createElement(PlaygroundPage)); fireEvent.change(screen.getByPlaceholderText(/ask finnic/i), { target: { value: 'create expense' } }); fireEvent.click(screen.getAllByRole('button').at(-1)!); await waitFor(() => expect(send).toHaveBeenCalled()); expect(mocks.executor).not.toHaveBeenCalled() })
