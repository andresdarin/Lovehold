...
export default function MessageList({ messages, sending, onSelectSuggestion, onResolveAction }: Props) {
  ...
  return <div ref={scroll} onScroll={() => { const el = scroll.current; if (el) atBottom.current = el.scrollHeight - el.scrollTop - el.clientHeight < 100 }}
    className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-6">
    {!messages.length ? <div className="mx-auto flex min-h-full max-w-md flex-col justify-center gap-6">
      <div><img src="/brand/finnic-mascot.png" alt="" className="h-28 w-28 object-contain" />
        <h2 className="mt-3 text-2xl font-semibold">Tus números, más claros.</h2>
        <p className="mt-2 text-base leading-relaxed text-muted-foreground">Revisemos tus movimientos y las opciones que tenés. Si querés registrar un gasto, te mostraré los datos antes de confirmarlo.</p>
      </div>
    ...
    </div> : <div role="log" aria-label="Conversación con Finnic" aria-live="polite" className="space-y-5">
      {messages.filter(msg => msg.role !== 'SYSTEM' && !msg.metadata?.functionCall && !msg.metadata?.functionResponse).map(msg => {
        ...
        return <article key={msg.id} className={user ? 'ml-auto max-w-[88%]' : 'max-w-full'}>
          <p className="mb-1.5 text-xs font-medium text-muted-foreground">{user ? 'Vos' : 'Finnic'}</p>
          <div className={user ? 'rounded-2xl rounded-tr-sm bg-primary px-4 py-3 text-primary-foreground' : 'rounded-2xl rounded-tl-sm border border-border bg-surface px-4 py-4'}>
            {label && <p className="mb-3 flex items-center gap-2 text-xs font-semibold text-muted-foreground"><Icon size={16} />{label}</p>}
            {user ? <p className="whitespace-pre-wrap break-words text-base">{msg.content}</p> : <div className="prose-finnic text-base"><FinnicMarkdown content={msg.content} /></div>}
            {pending && status === 'pending' && <div className="mt-4 flex flex-wrap gap-2">
              <button disabled={sending} onClick={() => onResolveAction(pending, 'confirm')} className="min-h-11 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50">Confirmar gasto</button>
              <button disabled={sending} onClick={() => onResolveAction(pending, 'cancel')} className="min-h-11 rounded-xl border border-border px-4 text-sm hover:bg-surface-soft disabled:opacity-50">Cancelar</button>
            </div>}
          </div>
        </article>
      })}
